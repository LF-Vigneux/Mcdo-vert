...
\begin{align*}
    ...
    \intertext{Posons tout les angles de mesures à 0.}
    &= H H H\ket{\psi_0}\\
    &= H\ket{\psi_0}
\end{align*}
...
\end{align*}
On veut que tout le terme entre crochet donne H pour annuler la porte Hadamard à gauche de ce terme. On sait et on peut vérifier aisément sous forme matricielle que $H = 1/\sqrt{2}(X + Z)$. Ainsi, on doit choisir les angles de fonction à ce que les termes en identité et en Y soit nuls tandis que les termes en X et Z aient un coefficient $1/\sqrt{2}$.
\\\\
...
\begin{align*}
    ...
    &= -i\ket{\psi_0}
\end{align*}
Ainsi, on a bel et bien obtenu une porte identité sur l'état initiale du qubit 0 pour l'état final du qubit 3. Cette porte est bonne à une phase globale près: $e^{-i\pi/2} = -i$. \ \ $\blacksquare$
...
\section*{4.}
Avec les éléments des précédentes questions, il est simple de choisir les bases de mesure sur les différents qubits. On remarque la structure de l'état grappe dans le circuit du dernier numéro.
...
\section*{7.}
Pour le code, il est possible d'ajuster les bases de mesure pour seulment utiliser des $R_Z$ comme portes conditonné sur des mesures antérieures. En effet, l'état du numéro 5 peut être réécrit comme:
\begin{flalign*}
    &X^{m_2}Z^{m_1}X^{m_0}H\bigg(R_z(-\theta_2\cdot (-1)^{m_1})\cdot HR_z(-\theta_1\cdot (-1)^{m_0})\cdot HR_z(-\theta_0)\ket{\psi_0}\bigg)\\
...
On obtient alors les mêmes angles qu'au numéro 5 sauf que les corrections d'angle sont faits par des rz contrôlés par la mesure comme il est montré ici.
